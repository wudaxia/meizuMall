		...
		f: function(data) {
			...
			if (_this.index !== 0 && _this.value[_this.index - 1] == "0" && this.default == 0) {
				...
				if (item.length > 0 && this.default == 1) {
					...
					var id = item[0].s_region_id || 0;
					focus = item[0].s_region_id;
					childData = item[0].child;
					if (!_this.value[this.index]) {
						_this.value[this.index] = id;
						_this.text[this.index] = item[0].s_region_name;
					}
					str += '<dd pid="' + pid + '" class="' + cls + '" ref="' + id + '">' + item[0].s_region_name + '</dd>';
				}
				for (var j = _this.default, len = item.length; j < len; j++) {
					var pid = item[j].pid || 0;
					var id = item[j].s_region_id || 0;
					var cls = '';
					...
					str += '<dd pid="' + pid + '" class="' + cls + '" ref="' + id + '">' + item[j].s_region_name + '</dd>';
				}
			}
			str += "</dl>";
			var newdom = $(str);
