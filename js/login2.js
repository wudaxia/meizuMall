...
function openUserInfo(){
    personalFoot();
   var oldPhone= JSON.parse(sessionStorage.getItem("oldPhone"));
    if(oldPhone!=null&&oldPhone!=undefined&&oldPhone!="")
    {
        $(".l-loginOldUser").val(oldPhone);
    }
    else{
        openmodal("您还未绑定手机，请先去绑定");
        setTimeout(function(){
...
function enters(btn){
    ...
    var oldUser=$(".l-loginOldUser").val();
    ...
    if(phoneText(user)==true)//判断手机号的格式
    {
        ...
        {
            var data='{"token":"'+token+'","phone":"'+user+'","code":"'+pass+'","old_phone":"'+oldUser +'"}';
            console.log(data);
            AjaxSubmit("post", JSON.parse(data), basePath + "User/amendUserPhone ",amendUserPhone_fun);
        }
        else {
            layer.closeAll();
            openmodal("请输入验证码");
...
//得到绑定结果
function amendUserPhone_fun(res){
    console.log(res);
    openmodal("更换绑定成功!");
    sessionStorage.removeItem("token");
    sessionStorage.setItem("oldPhone",JSON.stringify($(".l-loginUser").val()));
    setTimeout(function(){
