...
function openUserInfo(){
    ...
    if(token!=null&&token!=undefined&&token!="")
    {
        ...
        getMoreover();
        var pc_orderStyle=JSON.parse(sessionStorage.getItem("orderStyle2"));
...
function ol_styleChose(btn){
    ...
    sessionStorage.setItem("orderStyle2",JSON.stringify(orderStyle));
    getOrderList();
    layer.closeAll();
...
function getOrderList_fun(res){
    ...
    if(orderList!=null&&orderList!=undefined&&orderList!=""&&orderList.length>0)
    {
        ...
        $.each(orderList,function(i,orderLi){
            ...
            {
                statusWord="退货中";
                spanDiv+='<span class="ol-liActiveSpan ol-liActiveSpan1" data-remark="'+orderLi.remark+'"  data-orderStatus="'+status+'" data-orderNo="'+orderLi.order_no+'" data-id="0" onclick="event.cancelBubble = true;ol_active(this)">取消申请</span> ' +
                    '<a  onclick="event.cancelBubble = true" class="ol-liActiveSpan ol-liActiveSpan2" href="tel:15167500695" >联系客服</a> ';
            }
            else if(status=="2")
            {
                statusWord="退货完成";
                spanDiv+='<a onclick="event.cancelBubble = true" class="ol-liActiveSpan ol-liActiveSpan2" href="tel:15167500695" >联系客服</a> ';
            }
            else if(status=="3")
            {
                statusWord="退货失败";
                spanDiv+= '<span class="ol-liActiveSpan ol-liActiveSpan3" data-remark="'+orderLi.remark+'" data-orderStatus="'+status+'" data-orderNo="'+orderLi.order_no+'" data-id="5" onclick="event.cancelBubble = true;ol_active(this)">重新申请</span> ' +
                    '<a onclick="event.cancelBubble = true" class="ol-liActiveSpan ol-liActiveSpan2" href="tel:15167500695" >联系客服</a>';
            }
            //else if(status=="4")
            //{
            ...
            if(orderLi.remark=="2")
            {
                orderLiDiv+=  '<span class="fr">共'+productCount+'件商品&nbsp;应扣积分：<span class="ol-liSumMoney">'+parseInt(productSum)+'</span>分</span> ';
            }
            else{
                orderLiDiv+=  '<span class="fr">共'+productCount+'件商品&nbsp;应付金额：¥<span class="ol-liSumMoney">'+productSum+'</span></span> ';
            }
        ...
        if(pageCount>orderList.length)
        {
            ...
            if(pageIndex==1&&orderList.length ==0)
            {
                ...
                $(".o-blank").remove();
                recommend();
                var blank=$('<div class="o-blank"><img src="../img/blank_03.png"/><div class="o-blankWord">您还未有相关订单</div></div>')
                $(".ol-Ul").append(blank);
...
function ol_active(btn){
    ...
    if(ol_style=="0")
    {
        getConfirm2(orderNo,"取消申请吗？",orderStatus);
    }
    //立即付款
    else if(ol_style=="1")
    {
